...
feature_space_dir = '/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/DESeq2_logOdds_combinedFeatureSpaces/'
selected_feature_space_dir = '/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/phenotype_featureSpaces/'
comparisons_f = '/data/mstambou/proteome_landscapes/highly_abundant_genomes/comparisonLists.txt'
n_reps = 100

out_dir = '/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/modelComparisons/'

min_log_odds = 2.0
min_p_val = 0.05
top_n = 50
min_samples = 15
min_log_odds_selected = 2.5

if not os.path.isdir(out_dir):
    os.mkdir(out_dir)


n_features_dic = {'CD_subUC_vs_UC' : 110,
                  'healthy_vs_acute_leukemia': 230,
                  'healthy_vs_UC': 130,
                  'healthy_subUC_vs_UC' : 120,
                  'healthy_vs_acute_leukemia_subHealthy': 230,
                  'healthy_subT1D_vs_T1D' : 60,                  
                  'healthy_vs_CD': 165
                 }
...
with open(comparisons_f, 'r') as in_f:
    for I, line in enumerate(in_f):
        ...
        data_points_f = f'{feature_space_dir + phen1}_vs_{phen2}_combinedFeatureSpaces_minLogOdds_{min_log_odds}_minPval_{min_p_val}.tsv'
        ...
        selected_data_points_f = f'{selected_feature_space_dir + phen1}_vs_{phen2}/{phen1}_vs_{phen2}_featureSpaces_minLogOdds_{min_log_odds_selected}_top_n_{top_n}_minSamples_{min_samples}.tsv'
        ...
        lsvc = LinearSVC(C = 2.0, penalty="l1", dual=False, max_iter = 10000).fit(X, y)
