"""
script to create protein group profile matrices for DESeq2
"""
...
out_dir = '/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/phenotype_proteinGroup_abundance_profiles_DESeq2/'
...
def get_phenPair2protGroupProfiles(phen_pair, out_dir = out_dir, phen2proteinGroups_dic = phen2proteinGroups_dic):
    ...
    for sample in phen1_samples:
        ...
        sample_col = [1]*len(cluster2idx_dic)
        for i, row in sample_df.iterrows():
            clusterID = row['clusterID']
            n_specs = round(row['NSAF']*scaling_factor)
            if n_specs == 0:
                n_specs = 1
    ...
    metadata_df.to_csv(out_dir + f'{phen1}_vs_{phen2}_metadata.tsv', sep = '\t', index = False)
