...
expressed genomes accoss two classes of phenotypes and will use them as features to train an SVM and a RF later 
"""
import os
import pandas as pd
import json
...
in_dir = sys.argv[1]#'/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/proteinGroups_logOdds_phenotypes/'

phenotype_1 = sys.argv[2]#'healthy'
phenotype_2 = sys.argv[3]#'CD'

samples_dir = sys.argv[4]#'/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/cluster_NSAFs/'
out_dir = sys.argv[5]#'/data/mstambou/proteome_landscapes/cluster_abundances_HAPiID_6k/phenotype_featureSpaces/'
samples2phenotypes_dic_f = sys.argv[6]#'/data/mstambou/proteome_landscapes/highly_abundant_genomes/samplesSubSamples2phenotypes_dic.json'

min_logOdds = float(sys.argv[7])#2.5
...
phenotype1_only_f = f'{in_dir}{phenotype_1}_vs_{phenotype_2}/{phenotype_1}_onlyProteinGroups.tsv'
phenotype2_only_f = f'{in_dir}{phenotype_1}_vs_{phenotype_2}/{phenotype_2}_onlyProteinGroups.tsv'
...
print('number of differentially expressed protein groups', logOdds_df.shape[0])

filtered_logODds_df = logOdds_df[(logOdds_df['log_odds_ratio'] >= min_logOdds) | (logOdds_df['log_odds_ratio'] <= -min_logOdds)]
print('number of differentially expressed protein groups with absolute log2 odds of greater than', min_logOdds, ':', filtered_logODds_df.shape[0])
print(f'number of protein groups only expressed in {phenotype_1}, {len(phenotype1_only_df)}')
print(f'number of protein groups only expressed in {phenotype_2}, {len(phenotype2_only_df)}')
...
featured_proteinGroups = list(phen1_filtered_logOdds_df['clusterID'].loc[:top_n])
featured_proteinGroups.extend(list(phen2_filtered_logOdds_df['clusterID'].loc[:top_n]))
featured_proteinGroups.extend(list(phenotype1_only_df[phenotype1_only_df['n_samples'] >= min_samples]['clusterID']))
featured_proteinGroups.extend(list(phenotype2_only_df[phenotype2_only_df['n_samples'] >= min_samples]['clusterID']))

feature_space = copy.deepcopy(featured_proteinGroups)
...
for phenotype in phenotypes:
    ...
    for sample in phenotype_samples:
        print('processing sample', row_counter, sample, '...')
        sample_df = pd.read_csv(samples_dir + sample + in_suffix, sep = '\t')
        sample_row = [0.0]*len(feature_space_idx)
        for i, row in sample_df.iterrows():
            clusterID = row['clusterID']            
            if clusterID in featured_proteinGroups:
                rel_NSAF = row['NSAF_%']
                sample_row[feature_space_idx[clusterID]] = rel_NSAF
